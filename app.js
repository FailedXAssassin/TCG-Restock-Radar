...
async function loadFeed(){
  ...
  try{
    const feedToken=managerSecret()||googleToken;
    const res=await fetch(`${API_BASE}api/feed?t=${Date.now()}`,{cache:"no-store",headers:feedToken?{Authorization:`Bearer ${feedToken}`}:{}});
...
async function loadRecentlyMissed(){
  const list=$("#recentlyMissed");
  try{
    const res=await fetch(api("/api/recently-missed?t="+Date.now()),{cache:"no-store"});
    if(!res.ok)throw new Error();
    const items=(await res.json()).items||[];
    list.innerHTML="";
    if(!items.length){list.innerHTML="<small>No confirmed drops have sold out recently.</small>";return;}
    for(const item of items){
      const row=document.createElement("a");row.className="alert-row missed-alert";row.href=item.url||"#";row.target="_blank";row.rel="noopener";
      const title=document.createElement("strong");title.textContent=item.product||"Product";
      const detail=document.createElement("span");detail.textContent=`${item.store||"Retailer"} • was confirmed in stock, now sold out`;
      const time=document.createElement("time");time.textContent=item.created_at?`Dropped ${formatDropTime(item.created_at)}`:"Recently";
      row.append(title,detail,time);list.appendChild(row);
    }
  }catch(_){list.innerHTML="<small>Recently missed is temporarily unavailable.</small>";}
}
...
const GOOGLE_TOKEN_KEY="tcg-radar-google-token";
let googleToken=localStorage.getItem(GOOGLE_TOKEN_KEY)||sessionStorage.getItem(GOOGLE_TOKEN_KEY)||"", accountUser=null;
function saveGoogleToken(token){googleToken=token||""; if(googleToken){localStorage.setItem(GOOGLE_TOKEN_KEY,googleToken);sessionStorage.setItem(GOOGLE_TOKEN_KEY,googleToken);}else{localStorage.removeItem(GOOGLE_TOKEN_KEY);sessionStorage.removeItem(GOOGLE_TOKEN_KEY);}}
...
async function loadManagerControls(){
  ...
  try{
    ...
    managerRole=data.role||""; healthAllowed=true; $("#healthSection").hidden=activePage!=="radar"; loadHealth(); loadFeed(); localStorage.setItem(MANAGER_MODE_KEY,"1"); updateManagerButton(); $("#managerLogin").hidden=true; $("#ownerPinQuick").hidden=managerRole!=="owner"; $("#productForm").hidden=false; $("#ownerOnlyControls").hidden=managerRole!=="owner";
