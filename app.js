...
function signOut(){ saveGoogleToken(""); accountUser=null; $("#googleSignIn").innerHTML=""; $("#googleSignIn").hidden=false; configureGoogleSignIn(); renderPurchases(); }
...
async function loadOwnerPinStatus(){try{const data=await ownerFetch("/api/admin/owner-pin"); $("#ownerPinStatus").textContent=data.configured?"Owner PIN is set. Change it below anytime.":"No owner PIN set yet.";}catch(error){$("#ownerPinStatus").textContent=error.message;}}
$("#ownerPinForm").addEventListener("submit",async event=>{event.preventDefault(); const pin=$("#ownerPin").value.trim(); if(!/^\d{4,20}$/.test(pin)){$("#ownerMessage").textContent="Owner PIN must be 4–20 numbers.";return;} try{const result=await ownerFetch("/api/admin/owner-pin",{method:"PUT",body:JSON.stringify({pin})}); $("#ownerPin").value=""; $("#ownerPinStatus").textContent="Owner PIN is set."; $("#ownerMessage").textContent=result.message;}catch(error){$("#ownerMessage").textContent=error.message;}});
